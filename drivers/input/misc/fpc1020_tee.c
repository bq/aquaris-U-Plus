		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_dbg(dev, "%s:'%s' enter!\n", __func__, buf);
	set_clks(fpc1020, (*buf == '1'));
	return count;
}
...
static int fpc1020_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
...
static const struct i2c_device_id fpc1020_sensor_id[] =
{
    { "fpc1020", 0 },
    { }
};
...
static struct i2c_driver fpc1020_driver =
{
    .probe	= fpc1020_probe,
    .remove	= fpc1020_remove,
    .id_table	= fpc1020_sensor_id,
    .driver	= {
        .name = "fpc1020",
        .owner = THIS_MODULE,
...
{
	int rc = i2c_add_driver(&fpc1020_driver);
	if (!rc)
		pr_info("%s OK\n", __func__);
	else
		pr_err("%s %d\n", __func__, rc);
