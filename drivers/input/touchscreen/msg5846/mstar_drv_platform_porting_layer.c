...
extern bool mstar_is_ffbm; /*ckt guoyi add for ffbm mode 2015-10-20*/
...
static ssize_t _DrvPlatformLyrVirtualKeysShow(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    return sprintf(buf,
        __stringify(EV_KEY) ":" __stringify(KEY_MENU) ":50:1330:100:100"
...
static s32 _DrvPlatformLyrTouchPinCtrlInit(struct i2c_client *pClient)
{
    ...
    _gGpioRst = of_get_named_gpio_flags(pDeviceNode, "mstar,reset-gpio",	0, &nFlag);
...
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(bool nFlag)
{
    ...
    if (nFlag == is_Regu_enable)
    {
        DBG(&g_I2cClient->dev, "*** %s(already %d) ***\n", __func__, is_Regu_enable);
		return;
    }
...
//    DBG(&g_I2cClient->dev, "nId=%d, nX=%d, nY=%d\n", nId, nX, nY); // TODO : add for debug
    ...
    }/*ckt guoyi add for ffbm mode 2015-10-20*/
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
    input_set_capability(g_InputDevice, EV_KEY, KEY_WAKEUP);
