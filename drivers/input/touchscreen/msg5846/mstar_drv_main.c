...
static struct kobject *mstar_android_touch_kobj = NULL; 
...
char* DrvMainFirmwareGetVersion(void)
{
   // DBG("*** %s() _gFwVersion = %s ***\n", __func__, _gFwVersion);

    return (char*)_gFwVersion;
}

static ssize_t show_chipinfo(struct device *dev,struct device_attribute *attr, char *buf)
{
	char fmverison[16]="unKnown";
//	char verndorName[16]="unknown";
	char* tmp = NULL;

	//read the fw version	
	tmp = DrvMainFirmwareGetVersion();
	if (tmp != NULL)
	{
		strncpy(fmverison, tmp, (sizeof(fmverison)-1));
	}
	//end

	return sprintf(buf, "IC:msg5846,FW_ver:%s\n",fmverison);
}

static DEVICE_ATTR(info, 0444, show_chipinfo, NULL);

#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
ssize_t show_mstar_GestureWakeupMode(struct device *dev,struct device_attribute *attr,char *pBuffer)
{
    u32 nLength = 0;

#ifdef CONFIG_SUPPORT_64_TYPES_GESTURE_WAKEUP_MODE
    nLength = sprintf(pBuffer, "mstar_GestureWakeupMode = 0x%x, 0x%x\n", g_GestureWakeupMode[0], g_GestureWakeupMode[1]);
#else
    nLength = sprintf(pBuffer, "mstar_GestureWakeupMode = 0x%x\n", g_GestureWakeupMode[0]);
#endif //CONFIG_SUPPORT_64_TYPES_GESTURE_WAKEUP_MODE

    return nLength;
}

ssize_t store_mstar_GestureWakeupMode(struct device *dev,struct device_attribute *devattr, const char *pBuffer, size_t count)
{
	u32 nLength = 0;
	u32 nWakeupMode[2] = {0};
	...
	if (pBuffer != NULL)
	{
		...
		return -1;
#else
		nLength = sscanf(pBuffer, "%x", &nWakeupMode[0]);
#endif
...
static DEVICE_ATTR(wakeup_gesture_enable, 0664, show_mstar_GestureWakeupMode, store_mstar_GestureWakeupMode);
#endif

static struct attribute *msg22xx_attrs[] =
{
    &dev_attr_info.attr,
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
...
void DrvMainRemoveProcfsDirEntry(void)
{
	...
	if(mstar_android_touch_kobj){
        sysfs_remove_group(mstar_android_touch_kobj, msg22xx_attribute_group);
        kobject_put(mstar_android_touch_kobj);
        mstar_android_touch_kobj = NULL;
        //DBG("Remove mstar_android_touch_kobj file node OK!\n");
    }
}
...
static s32 _DrvMainCreateProcfsDirEntry(void)
{
	...
	mstar_android_touch_kobj = kobject_create_and_add("android_touch", NULL);
    if (mstar_android_touch_kobj) {
        nRetVal = sysfs_create_group(mstar_android_touch_kobj, msg22xx_attribute_group);
        if (nRetVal != 0)
        {
            //DBG("*** msg22xx_attribute sysfs_create_file() failed, nRetVal = %d ***\n", nRetVal);
        }
    }
    return nRetVal;
