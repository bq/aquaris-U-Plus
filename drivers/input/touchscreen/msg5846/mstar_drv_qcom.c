...
bool is_Regu_enable = false;
#endif //CONFIG_ENABLE_REGULATOR_POWER_ON

bool mstar_is_ffbm = false; /*ckt guoyi add for ffbm mode 2015-10-20*/
        ...
        const struct i2c_device_id *id)
{
    ...
    ret = regulator_set_voltage(g_ReguVdd, 2850000, 2850000);
    if (ret)
    {
    ...
    is_Regu_enable = false;
#endif //CONFIG_ENABLE_REGULATOR_POWER_ON

    if (strstr(saved_command_line, "ffbm-01")) {
        printk(" *** bootmode = ffbm ! *** \n");
        mstar_is_ffbm = true;
