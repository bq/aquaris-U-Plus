...
static ssize_t mip_sys_fw_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(mip_get_fw_version(info, rbuf, sizeof(rbuf))){
...
*/
#ifdef DEBUG	
static DEVICE_ATTR(fw_update_section, 0444, mip_sys_fw_update_section, NULL);
#endif
static DEVICE_ATTR(fw_version, 0444, mip_sys_fw_version, NULL);
static DEVICE_ATTR(fw_path, 0664, mip_sys_fw_path_show, mip_sys_fw_path_store);
static DEVICE_ATTR(info, 0444, mip_sys_info, NULL);
#ifdef DEBUG	
static DEVICE_ATTR(power_on, 0444, mip_sys_power_on, NULL);
static DEVICE_ATTR(power_off, 0444, mip_sys_power_off, NULL);
static DEVICE_ATTR(reboot, 0444, mip_sys_reboot, NULL);
#endif
static DEVICE_ATTR(mode_glove, 0664, mip_sys_mode_show, mip_sys_mode_store);
static DEVICE_ATTR(mode_charger, 0664, mip_sys_mode_show, mip_sys_mode_store);
static DEVICE_ATTR(mode_cover_window, 0664, mip_sys_mode_show, mip_sys_mode_store);
static DEVICE_ATTR(mode_palm_rejection, 0664, mip_sys_mode_show, mip_sys_mode_store);
static DEVICE_ATTR(mode_edge, 0664, mip_sys_mode_show, mip_sys_mode_store);
static DEVICE_ATTR(image_intensity, 0444, mip_sys_image, NULL);
static DEVICE_ATTR(image_rawdata, 0444, mip_sys_image, NULL);
static DEVICE_ATTR(test_cm_delta, 0444, mip_sys_test, NULL);
static DEVICE_ATTR(test_cm_abs, 0444, mip_sys_test, NULL);
static DEVICE_ATTR(test_cm_jitter, 0444, mip_sys_test, NULL);
#if ((CHIP_MODEL == CHIP_MIT200) || (CHIP_MODEL == CHIP_MIT300))
static DEVICE_ATTR(test_short, 0444, mip_sys_test, NULL);
static DEVICE_ATTR(test_short2, 0444, mip_sys_test, NULL);
#else
static DEVICE_ATTR(test_short, 0444, mip_sys_test_short, NULL);
#endif
#if MIP_USE_WAKEUP_GESTURE
static DEVICE_ATTR(wakeup_gesture, 0444, mip_sys_wakeup_gesture, NULL);
#endif
